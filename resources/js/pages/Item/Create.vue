...
const form = useForm({
  item_type_id: null as string | null,
    ...
    <div class="mx-auto max-w-3xl space-y-8 px-4 sm:px-6 lg:px-8">
      ...
      <section class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        <header class="border-b border-emerald-800/20 bg-emerald-700 px-6 py-6 text-white sm:px-8">
          ...
          <h1 class="mt-2 text-2xl font-semibold tracking-tight">Create item</h1>
          <p class="mt-2 max-w-xl text-sm text-emerald-100">
            Define a cashflow item and the transactions that will feed your statement.
          </p>
        </header>
        ...
        <form
          ...
          @submit.prevent="
            form.submit(store().method, store().url, {
              preserveState: true,
              except: ['itemTypes', 'flows', 'frequencies'],
            })
          "
        >
            ...
            <div class="grid gap-5 sm:grid-cols-2">
              <div>
                ...
                <select id="flow" v-model="form.flow" required :class="selectClass">
                  <option disabled value="">Select flow…</option>
